...
### increase cpus more in the event using bigger matrices. for 400 1 is fine
...
def submit_collect_after(jobid: str, *, dataset_path: str, feature_dir: str, task: str,
                         ...
                         script_path: str = "/well/margulies/users/mnk884/DistanceValidationOx2025/brain2behaviour/src/brain2behaviour/cluster_scripts/CollectFeaturesandPredict-Aging.py") -> str:
...
def main():
    ...
    ap.add_argument("--select-script", default="/well/margulies/users/mnk884/DistanceValidationOx2025/brain2behaviour/src/brain2behaviour/cluster_scripts/select_features_batch-Aging.py")
    args = ap.parse_args()
    ...
    for j in range(n_perms):
        ...
        f = out_dir / f"ds_permed{j:04d}r_vals.csv"
        if f.exists():
            print()
            pass
        else:       
            ...
            with worklist.open("w") as fh:
                for fold in folds:
                    fh.write(f"python {args.select_script} --dataset {out_perm_path} --fold {fold} \n")
            ...
            try:
                ...
                print(f"[WARN] Array submission failed for perm {j:04d}: {e}")
                with open("failed_arrays.txt", "a") as fh:
                    fh.write(f"{j}\n")
                continue   # skip this perm and move on
            ...
            try:
                ...
                print(f"[WARN] Collect submission failed for perm {j:04d}: {e}")
                with open("failed_collects.txt", "a") as fh:
                    fh.write(f"{j}\n")
                continue
