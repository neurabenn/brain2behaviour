...
def submit_collect_after(jobid: str, *, dataset_path: str, feature_dir: str, task: str,
                         ...
                         script_path: str = "/well/margulies/users/mnk884/DistanceValidationOx2025/brain2behaviour/src/brain2behaviour/cluster_scripts/CollectFeaturesandPredict.py") -> str:
    ...
    for j in range(n_perms):
        ...
        if f.exists():
            ...
            with worklist.open("w") as fh:
                for fold in folds:
                    fh.write(f"python {args.select_script} --dataset {out_perm_path} --fold {fold} --passthrough_cols \n")
            ...
            # except Exception as e:
            #     print(f"[WARN] Collect submission failed for perm {j:04d}: {e}")
            #     # Optionally log to a file for later retry
            #     with open("failed_collects.txt", "a") as fh:
            #         fh.write(f"{j}\n")
            #     continue
