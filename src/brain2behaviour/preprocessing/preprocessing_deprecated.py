...
class PrepConfounds(BaseEstimator, TransformerMixin):
    def __init__(
        ...
        noop_cols : list of str, optional
            Columns to do nothing to. 
    ...
    def transform(self, X, y=None):
        ...
        if self.add_squares:
            if self.verbose==True:
                print(f'there are {len(self.encode_cols)} encoded columns. encoded columns will not squared')
            sq = X_enc.iloc[:, len(self.encode_cols):] ** 2
...
def clean_fold(dataset,
        ...
        prepTrainingConfs = PrepConfounds(subject_list=trainSubjs,encode_cols=encode_cols,
                                         bin_encode=bin_encode,area_cols=area_cols,volume_cols=volume_cols,
                                         gaussianize=gaussianize,add_squares=add_squares,zscore_cols=zscore_cols,verbose=True)

        prepTrainingConfs.fit(dataset.confounds)
        processedTrainingconfounds = prepTrainingConfs.transform(dataset.confounds)
        
        
        beh_cleanTrain=BehaviorPrep().fit_transform(dataset.behaviorData.loc[trainSubjs],processedTrainingconfounds)
        brain_cleanTrain=DistancePrep().fit_transform(dataset.brainData.loc[trainSubjs],processedTrainingconfounds)
        
        # #### testing half -- split to avoid leakage of training mean with zscoring
        testSubjs=dataset.cv_folds[fold]['testing']
        prepTestingConfs=PrepConfounds(subject_list=testSubjs,encode_cols=encode_cols,
                                      bin_encode=bin_encode,area_cols=area_cols,volume_cols=volume_cols,
                                      gaussianize=gaussianize,add_squares=add_squares,zscore_cols=zscore_cols)
        

        prepTestingConfs.fit(dataset.confounds)
        processedTestingconfounds = prepTestingConfs.transform(dataset.confounds)
        beh_cleanTest=BehaviorPrep().fit_transform(dataset.behaviorData.loc[testSubjs],processedTestingconfounds)    
        brain_cleanTest=DistancePrep().fit_transform(dataset.brainData.loc[testSubjs],processedTestingconfounds)
